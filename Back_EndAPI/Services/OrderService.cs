...
public class OrderService
{
    ...
    public async Task<OrderStatusResponse?> GetStatusAsync(int orderId)
    {
        ...
        var items = await _context.SoldItems
            .Where(s => s.CustomerOrderId == orderId)
            .Select(s => new OrderItemResponse
            {
                ProductId = s.SkuNumber,
                Sku = s.SkuNumber.ToString(),
                Quantity = s.Qty
            })
            .ToListAsync();
        ...
        {
            OrderId = order.Id,
            CreatedAt = order.DateOrdered.ToDateTime(TimeOnly.MinValue),
            Items = items
        };
    }
    ...
    public async Task<OrderResponse> PickAsync(int orderId)
    {
        ...
        var lines = await _context.SoldItems
